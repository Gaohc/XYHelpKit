...
typedef IMP (^XYYSWizzleImpProvider)(void);

static const char XYYSwizzledDeallocKey;

@interface XYYSwizzleObject()
@property (nonatomic,readwrite,copy) XYYSWizzleImpProvider impProviderBlock;
@property (nonatomic,readwrite,assign) SEL selector;
@end

@implementation XYYSwizzleObject

- (XYYSwizzleOriginalIMP)getOriginalImplementation{
    NSAssert(_impProviderBlock,nil);
    return (XYYSwizzleOriginalIMP)_impProviderBlock();
}
...
BOOL XYY_requiresDeallocSwizzle(Class class)
{
    BOOL swizzled = NO;
    
    for ( Class currentClass = class; !swizzled && currentClass != nil; currentClass = class_getSuperclass(currentClass) ) {
        swizzled = [objc_getAssociatedObject(currentClass, &XYYSwizzledDeallocKey) boolValue];
    }
...
void XYY_swizzleDeallocIfNeeded(Class class)
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        cleanupSEL = sel_getUid("XYY_cleanKVO");
    });
    
    @synchronized (class) {
        if ( !XYY_requiresDeallocSwizzle(class) ) {
            return;
        }
        
        objc_setAssociatedObject(class, &XYYSwizzledDeallocKey, @(YES), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
...
void __JJ_SWIZZLE_BLOCK(Class classToSwizzle,SEL selector,XYYSwizzledIMPBlock impBlock){
    Method method = class_getInstanceMethod(classToSwizzle, selector);
    
    __block IMP originalIMP = NULL;
    
    XYYSWizzleImpProvider originalImpProvider = ^IMP{
    ...
    XYYSwizzleObject* swizzleInfo = [XYYSwizzleObject new];
