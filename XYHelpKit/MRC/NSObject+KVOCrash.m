...
@interface XYYObserverContainer : NSObject
...
@implementation XYYObserverContainer
...
- (void)hookAddObserver:(NSObject *)observer forKeyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options context:(void *)context{
    ...
    XYYObserverContainer* observerContainer = objc_getAssociatedObject(observer,&ObserverDeallocKVOKey);
    
    if (!observerContainer) {
        observerContainer = [XYYObserverContainer new];
    ...
    XYY_swizzleDeallocIfNeeded(self.class);
    XYY_swizzleDeallocIfNeeded(observer.class);
}
...
- (void)hookRemoveObserver:(NSObject *)observer forKeyPath:(NSString *)keyPath{
     ...
     * bug link:https://github.com/jezzmemo/XYYException/issues/68
     */
...
- (void)XYY_cleanKVO{
    ...
    KVOObjectContainer* objectContainer = objc_getAssociatedObject(self, &DeallocKVOKey);
    XYYObserverContainer* observerContainer = objc_getAssociatedObject(self, &ObserverDeallocKVOKey);
